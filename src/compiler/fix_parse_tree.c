 ...
 * -------------------------------------------------------------------------
 *    File name: fix_parse_tree.c
 *      Version: v0.0.0
 *   Created on: 2015-05-05 14:14:03 by konyka
 *  Modified by: konyka
...
static expression_t *
fix_identifier_expression(block_t *current_block, expression_t *expr)
{
    declaration_tag *decl;
    function_definition_t *fd;

    decl = poc_search_declaration(expr->u.identifier.name, current_block);
    if (decl) {
        expr->type = decl->type;
        expr->u.identifier.is_function = POVM_FALSE;
        expr->u.identifier.u.declaration = decl;
        return expr;
    }
    fd = poc_search_function(expr->u.identifier.name);
    if (fd == NULL) {
        poc_compile_error(expr->line_number,
    ...
    expr->u.identifier.is_function = POVM_TRUE;
    expr->u.identifier.u.function = fd;

    return expr;
}

static expression_t *fix_expression(block_t *current_block, expression_t *expr);

static expression_t *
fix_comma_expression(block_t *current_block, expression_t *expr)
{
    expr->u.comma.left
...
static void
cast_mismatch_error(int line_number, povm_basic_type_enum src, povm_basic_type_enum dest)
{
    char *src_name = poc_get_basic_type_name(src);
    char *dest_name = poc_get_basic_type_name(dest);
...
static expression_t *
alloc_cast_expression(cast_type_enum cast_type, expression_t *operand)
{
    expression_t *cast_expr = poc_alloc_expression(CAST_EXPRESSION);
    ...
    if (cast_type == INT_TO_DOUBLE_CAST) {
        cast_expr->type = poc_alloc_type_specifier(POVM_DOUBLE_TYPE);
    } else if (cast_type == DOUBLE_TO_INT_CAST) {
        cast_expr->type = poc_alloc_type_specifier(POVM_INT_TYPE);
    } else if (cast_type == BOOLEAN_TO_STRING_CAST
               || cast_type == INT_TO_STRING_CAST
               || cast_type == DOUBLE_TO_STRING_CAST) {
        cast_expr->type = poc_alloc_type_specifier(POVM_STRING_TYPE);
    }
...
static expression_t *
create_assign_cast(expression_t *src, type_specifier_t *dest)
{
    expression_t *cast_expr;

    if (src->type->derive != NULL
        || dest->derive != NULL) {
        poc_compile_error(src->line_number, DERIVE_TYPE_CAST_ERR,
                          MESSAGE_ARGUMENT_END);
    }
    if (src->type->basic_type == dest->basic_type) {
        return src;
    }
    if (src->type->basic_type == POVM_INT_TYPE
        && dest->basic_type == POVM_DOUBLE_TYPE) {
        cast_expr = alloc_cast_expression(INT_TO_DOUBLE_CAST, src);
        return cast_expr;
    } else if (src->type->basic_type == POVM_DOUBLE_TYPE
               && dest->basic_type == POVM_INT_TYPE) {
...
static expression_t *
fix_assign_expression(block_t *current_block, expression_t *expr)
{
    expression_t *operand;

    if (expr->u.assign_expression.left->kind != IDENTIFIER_EXPRESSION) {
        poc_compile_error(expr->u.assign_expression.left->line_number,
...
static expression_t *
eval_math_expression_int(expression_t *expr, int left, int right)
{
    if (expr->kind == ADD_EXPRESSION) {
        ...
        debug_assert(0, ("expr->kind..%d\n", expr->kind));
    }
    expr->kind = INT_EXPRESSION;
    expr->type = poc_alloc_type_specifier(POVM_INT_TYPE);

    return expr;
}

static expression_t *
eval_math_expression_double(expression_t *expr, double left, double right)
{
    if (expr->kind == ADD_EXPRESSION) {
    ...
    expr->type = poc_alloc_type_specifier(POVM_DOUBLE_TYPE);

    return expr;
}

static expression_t *
chain_string(expression_t *expr)
{
    povm_char *left_str = expr->u.binary_expression.left->u.string_value;
    povm_char *right_str;
    int         len;
    povm_char    *new_str;

    right_str = poc_expression_to_string(expr->u.binary_expression.right);
    if (!right_str) {
        return expr;
    }
    ...
    expr->type = poc_alloc_type_specifier(POVM_STRING_TYPE);
...
static expression_t *
eval_math_expression(block_t *current_block, expression_t *expr)
{
...
static expression_t *
cast_binary_expression(expression_t *expr)
{
    expression_t *cast_expression;

    if (poc_is_int(expr->u.binary_expression.left->type)
        && poc_is_double(expr->u.binary_expression.right->type)) {
        cast_expression
            = alloc_cast_expression(INT_TO_DOUBLE_CAST, 
                                    expr->u.binary_expression.left);
        expr->u.binary_expression.left = cast_expression;

    } else if (poc_is_double(expr->u.binary_expression.left->type)
               && poc_is_int(expr->u.binary_expression.right->type)) {
        cast_expression
            = alloc_cast_expression(INT_TO_DOUBLE_CAST, 
                                    expr->u.binary_expression.right);
        expr->u.binary_expression.right = cast_expression;

    } else if (poc_is_string(expr->u.binary_expression.left->type)
               && poc_is_boolean(expr->u.binary_expression.right->type)) {
        cast_expression
...
static expression_t *
fix_math_binary_expression(block_t *current_block, expression_t *expr)
{
    expr->u.binary_expression.left
    ...
    if (poc_is_int(expr->u.binary_expression.left->type)
        ...
        expr->type = poc_alloc_type_specifier(POVM_DOUBLE_TYPE);
               ...
               && poc_is_string(expr->u.binary_expression.right->type)) {
        expr->type = poc_alloc_type_specifier(POVM_STRING_TYPE);
    } else {
        poc_compile_error(expr->line_number,
...
static expression_t *
eval_compare_expression_boolean(expression_t *expr,
                                povm_boolean_enum left, povm_boolean_enum right)
{
    ...
    expr->type = poc_alloc_type_specifier(POVM_BOOLEAN_TYPE);

    return expr;
}

static expression_t *
eval_compare_expression_int(expression_t *expr, int left, int right)
{
    if (expr->kind == EQ_EXPRESSION) {
        expr->u.boolean_value = (left == right);
    } else if (expr->kind == NE_EXPRESSION) {
...
static expression_t *
eval_compare_expression_double(expression_t *expr, int left, int right)
{
    if (expr->kind == EQ_EXPRESSION) {
        expr->u.boolean_value = (left == right);
    } else if (expr->kind == NE_EXPRESSION) {
...
static expression_t *
eval_compare_expression_string(expression_t *expr,
                               povm_char *left, povm_char *right)
{
    int cmp;
...
static expression_t *
eval_compare_expression(expression_t *expr)
{
...
static expression_t *
fix_compare_expression(block_t *current_block, expression_t *expr)
{
    expr->u.binary_expression.left
        = fix_expression(current_block, expr->u.binary_expression.left);
    expr->u.binary_expression.right
...
static expression_t *
fix_logical_and_or_expression(block_t *current_block, expression_t *expr)
{
    expr->u.binary_expression.left
        = fix_expression(current_block, expr->u.binary_expression.left);
    expr->u.binary_expression.right
    ...
    if (poc_is_boolean(expr->u.binary_expression.left->type)
        && poc_is_boolean(expr->u.binary_expression.right->type)) {
        expr->type = poc_alloc_type_specifier(POVM_BOOLEAN_TYPE);
    } else {
        poc_compile_error(expr->line_number,
...
static expression_t *
fix_minus_expression(block_t *current_block, expression_t *expr)
{
    expr->u.minus_expression
        = fix_expression(current_block, expr->u.minus_expression);

    if (!poc_is_int(expr->u.minus_expression->type)
        && !poc_is_double(expr->u.minus_expression->type)) {
        poc_compile_error(expr->line_number,
...
static expression_t *
fix_logical_not_expression(block_t *current_block, expression_t *expr)
{
    expr->u.logical_not
    ...
    if (!poc_is_boolean(expr->u.logical_not->type)) {
        poc_compile_error(expr->line_number,
...
static void
check_argument(block_t *current_block, function_definition_t *fd, expression_t *expr)
{
...
static expression_t *
fix_function_call_expression(block_t *current_block, expression_t *expr)
{
    ...
    fd = poc_search_function(func_expr->u.identifier.name);
    if (fd == NULL) {
        poc_compile_error(expr->line_number,
    ...
    expr->type = poc_alloc_type_specifier(fd->type->basic_type);
...
static expression_t *
fix_inc_dec_expression(block_t *current_block, expression_t *expr)
{
    expr->u.inc_dec.operand
        = fix_expression(current_block, expr->u.inc_dec.operand);

    if (!poc_is_int(expr->u.inc_dec.operand->type)) {
        poc_compile_error(expr->line_number,
...
static expression_t *
fix_expression(block_t *current_block, expression_t *expr)
{
    if (expr == NULL)
        return NULL;
    ...
    default:
        debug_assert(0, ("bad case. kind..%d\n", expr->kind));
    }

    return expr;
}

static void add_local_variable(function_definition_t *fd, declaration_tag *decl)
{
    fd->local_variable
        = mem_realloc(fd->local_variable,
                      sizeof(declaration_tag*) * (fd->local_variable_count+1));
    fd->local_variable[fd->local_variable_count] = decl;
    decl->variable_index = fd->local_variable_count;
    fd->local_variable_count++;
}

static void fix_statement_list(block_t *current_block, statement_list_t *list,
                               function_definition_t *fd);

static void
fix_if_statement(block_t *current_block, if_statement_tag *if_s,
                 function_definition_t *fd)
{
    elsif_t *pos;
...
static void
fix_return_statement(block_t *current_block, return_statement_tag *return_s,
                     function_definition_t *fd)
{
    expression_t *return_value;
    expression_t *casted_expression;

    return_value
        = fix_expression(current_block, return_s->return_value);

    if (return_value == NULL) {
        debug_assert(fd->type->derive == NULL, ("closure is not supported."));
        switch (fd->type->basic_type) {
        case POVM_BOOLEAN_TYPE:
            return_value = poc_alloc_expression(BOOLEAN_EXPRESSION);
            return_value->u.boolean_value = POVM_FALSE;
            break;
        case POVM_INT_TYPE:
            return_value = poc_alloc_expression(INT_EXPRESSION);
            return_value->u.int_value = 0;
            break;
        case POVM_DOUBLE_TYPE:
            return_value = poc_alloc_expression(DOUBLE_EXPRESSION);
            return_value->u.int_value = 0.0;
            break;
        case POVM_STRING_TYPE:
            return_value = poc_alloc_expression(STRING_EXPRESSION);
...
static void
add_declaration(block_t *current_block, declaration_tag *decl,
                function_definition_t *fd, int line_number)
{
    if (poc_search_declaration(decl->name, current_block)) {
        poc_compile_error(line_number,
    ...
    if (current_block) {
        current_block->declaration_list
            = poc_chain_declaration(current_block->declaration_list, decl);
        add_local_variable(fd, decl);
        decl->is_local = POVM_TRUE;
    } else {
        poc_compiler_t *compiler = poc_get_current_compiler();
        compiler->declaration_list
            = poc_chain_declaration(compiler->declaration_list, decl);
        decl->is_local = POVM_FALSE;
    }
}

static void
fix_statement(block_t *current_block, statement_t *statement,
              function_definition_t *fd)
{
...
static void
fix_statement_list(block_t *current_block, statement_list_t *list,
                   function_definition_t *fd)
{
...
static void
add_parameter_as_declaration(function_definition_t *fd)
{
    declaration_tag *decl;
    parameter_list_t *param;

    for (param = fd->parameter; param; param = param->next) {
        if (poc_search_declaration(param->name, fd->block)) {
            poc_compile_error(param->line_number,
                              PARAMETER_MULTIPLE_DEFINE_ERR,
                              STRING_MESSAGE_ARGUMENT, "name", param->name,
                              MESSAGE_ARGUMENT_END);
        }
        decl = poc_alloc_declaration(param->type, param->name);
        add_declaration(fd->block, decl, fd, param->line_number);
    }
}

static void
add_return_function(function_definition_t *fd)
{
    statement_list_t *last;
    statement_list_t **last_p;
    statement_t *ret;
    ...
    ret = poc_create_return_statement(NULL);
    fix_return_statement(fd->block, &ret->u.return_s, fd);
    *last_p = poc_create_statement_list(ret);
}

void
poc_fix_tree(poc_compiler_t *compiler)
{
    function_definition_t *func_pos;
